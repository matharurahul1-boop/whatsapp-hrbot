...
function JoinForm() {
  ...
  let orgId = '';
  try { orgId = atob(rawOrg); } catch { orgId = rawOrg; }

  const role = ['employee', 'manager', 'hr'].includes(rawRole) ? rawRole : 'employee';
  ...
  useEffect(() => {
    if (!orgId) { setChecking(false); setError('Invalid invite link.'); return; }
    fetch(`/api/organizations/info?id=${encodeURIComponent(orgId)}`)
      .then(r => r.json())
